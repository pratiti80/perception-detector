...
import time
from config import REAL_CAR_WIDTH, FOCAL_LENGTH, DATA_PATH, TTC_MID, TTC_UPPER

# Load a lightweight, fast model
model = YOLO("yolov8n.pt") 

# Open test dashcam footage
...
while cap.isOpened():
    ...
    for box in results[0].boxes:
        ...
        if bbox_width > 0:
            ...
            # --- DRAWING THE 3D BOX ---
            if ttc == float('inf') or ttc > TTC_UPPER:
                color = (0, 255, 0) 
            elif TTC_MID < ttc <= TTC_UPPER:
                color = (0, 165, 255)
            elif ttc <= TTC_MID:
                color = (0, 0, 255)
            thickness = 2
